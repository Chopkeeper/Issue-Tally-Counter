...
// Declare the XLSX library as a global variable.
// It's loaded from a <script> tag in index.html to avoid module resolution issues.
...
const Dashboard: React.FC<DashboardProps> = ({ dataForMonth, departments, issueTypes }) => {
  ...
  const handleExportToExcel = () => {
    ...
    const dataRows = departmentsWithData.map(dept => {
      ...
      // Fix: Explicitly type `rowData` to allow both string and number types, resolving the error.
    ...
    <div className="w-full max-w-4xl mx-auto bg-gray-800/50 rounded-2xl p-6 shadow-2xl shadow-cyan-500/10">
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          ...
          className="bg-green-600 hover:bg-green-500 text-white font-bold py-2 px-4 rounded-lg transition-colors text-sm flex items-center gap-2 disabled:bg-gray-500 disabled:cursor-not-allowed disabled:hover:bg-gray-500"
